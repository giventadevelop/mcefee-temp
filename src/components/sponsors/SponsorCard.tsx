...
export function SponsorContactSocialIconRow({
  sponsor,
  stopPropagation,
}: {
  ...
  return (
    <div
      className="mh-social-icon-row flex flex-nowrap items-center justify-start gap-2 overflow-x-auto"
    ...
    >
      {sponsor.contactEmail?.trim() && (
        <SocialIconLink
          ...
          external={false}
          onClick={onLinkClick}
          ariaLabel={`Email ${sponsor.contactEmail.trim()}`}
        />
      )}
      {sponsor.contactPhone?.trim() && (
        <SocialIconLink
          ...
          external={false}
          onClick={onLinkClick}
          ariaLabel={`Call ${sponsor.contactPhone.trim()}`}
        />
      )}
      {websiteHref && (
        <SocialIconLink platform="website" href={websiteHref} onClick={onLinkClick} />
      )}
