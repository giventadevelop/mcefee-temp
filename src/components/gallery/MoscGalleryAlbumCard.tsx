...
function ViewAlbumCtaModernist({
  onViewAlbum,
  disabled,
}: {
  onViewAlbum?: () => void;
...
function ModernistAlbumCard(props: MoscGalleryAlbumCardProps) {
  const {
    title,
    coverImageUrl,
    totalMediaCount,
  ...
  const openAlbum = () => {
    if (!viewDisabled && onViewAlbum) onViewAlbum();
  };

  const media = (
    <figure
      className={`mh-event-card-media ${onViewAlbum && !viewDisabled ? 'mh-gallery-card-media--clickable' : ''}`}
      onClick={onViewAlbum && !href ? openAlbum : undefined}
      role={onViewAlbum && !href ? 'button' : undefined}
      tabIndex={onViewAlbum && !href ? 0 : undefined}
      onKeyDown={
        onViewAlbum && !href
          ? (e) => {
      ...
      aria-label={onViewAlbum && !href ? 'View Album' : undefined}
    >
      {coverImageUrl ? (
        <Image
          src={coverImageUrl}
  ...
  const body = (
    <div className="mh-event-card-body">
      <div className="mh-event-card-meta">
        <span className="mh-event-card-date">{eventDateLine || 'Album'}</span>
        {categoryDisplayName && (
          <span className="mh-event-card-admission">{categoryDisplayName}</span>
        )}
      </div>

      <h3>{title}</h3>

      {description && <p className="mh-event-card-caption">{description}</p>}

      <div className="mh-event-card-actions">
        {href ? (
          <span className="mh-btn mh-btn-details">
            <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
          ...
          <ViewAlbumCtaModernist onViewAlbum={onViewAlbum} disabled={viewDisabled} />
        )}
      </div>
    </div>
  );
  ...
  if (href) {
    return (
      <Link href={href} className="mh-event-card">
        {media}
        {body}
      </Link>
    );
...
export function MoscGalleryAlbumCard(props: MoscGalleryAlbumCardProps) {
  const {
    title,
    coverImageUrl,
    totalMediaCount,
  ...
  if (variant === 'main-gallery') {
    return <ModernistAlbumCard {...props} />;
  }
        ...
        {coverImageUrl ? (
          ...
          >
            <div className="absolute inset-0 flex items-center justify-center opacity-10">
              <svg className="w-32 h-32 text-syro-red" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                <path d="M10 2h4v7h7v4h-7v9h-4v-9H3v-4h7V2z" />
              </svg>
            </div>
            <svg
              className="w-16 h-16 relative z-10 text-syro-red/50"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
              aria-hidden="true"
      ...
      <div className="p-5 sm:p-6 flex flex-col flex-1">
        {categoryDisplayName && (
          <div className="mb-2.5">
            <span className="inline-block px-3 py-1 text-xs font-medium rounded-full bg-[#fce4ec] text-[#be1929] font-syro-primary">
              {categoryDisplayName}
            </span>
          </div>
        )}
        ...
        {eventDateLine && (
          <p className="text-sm mb-4 font-syro-primary text-gray-500">{eventDateLine}</p>
        )}
        ...
        <div className="flex items-center mt-auto">
          <ViewAlbumCtaMosc onViewAlbum={onViewAlbum} disabled={viewDisabled} />
        </div>
      </div>
    </>
  );
  ...
  if (href) {
    return (
      <Link href={href} className={cardOuterClass}>
        {cardContent}
      </Link>
    );
  }

  return <div className={cardOuterClass}>{cardContent}</div>;
}
